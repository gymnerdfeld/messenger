...
class API:
    ...
    >>> response.status
    '200 OK'
    >>> data = {"name": "Betsy", "age": 34, "very_smart": True}
    >>> response = client.put("/", data=json.dumps(data))
    >>> response.status
    '422 UNPROCESSABLE ENTITY'
    ...
    def register(self, method, url, func=None):
        ...
        mismatch = url_params ^ func_url_params
        if mismatch:
            raise TypeError(
                f"{func.__name__}() arguments and route parameter mismatch "
    ...
    def __call__(self, environ, start_response):
        try:
            ...
            if isinstance(response, HTTPException):
                # FIXME: Warn about not raising an exception
                raise response
...
def _simple_wrapper(func):
    sig = inspect.signature(func)
    parameter_keys = tuple(sig.parameters.keys())
    func_takes_request = sig.parameters and parameter_keys[0] == "request"
    if parameter_keys and "request" in parameter_keys[1:]:
        ...
        # FIXME: Warning        

    @functools.wraps(func)
    def wrapper(request, *args, **kwargs):
        if func_takes_request:
            # FIXME: fixme!!!
...
class PubSub:
    ...
    def subscribe(self, topic=None):
        ...
        Optionally a `topic` can be specified
        """
        q = queue.Queue(100)
        with self._main_lock:
            queues = self._queues[topic]
